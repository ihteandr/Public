  ...
  this.parser.on('frame', function (frame) {
    ...
    if (frame.type === 'SYN_STREAM') {
      ...
      // Destroy stream if we was asked to do this
        ...
        stream._rstCode = 0;
        if (frame.status === 5) {
          // If testing "cancels" connection - close stream and
...
function Stream(connection, frame) {
  ...
  // How much data can be sent TO testing before next WINDOW_UPDATE
  ...
  // How much data can be sent BY testing before next WINDOW_UPDATE
...
// Close stream if it was closed by both server and testing
//
