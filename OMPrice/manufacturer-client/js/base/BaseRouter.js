        ...
        * */
        ...
        _applyListener: function($scope, hash, isAfterChangeListener){
            ...
            for(var route in this.routes){
                ...
                if(checker){
                    listener = this[this.routes[route][type]];
                    break;
                }
            }
        ...
        run: function(){
            ...
            if(this.angModule){
                this.angModule.config(["$routeProvider", function($routeProvider){
                    for(var route in _self.routes){
                        var options = _self.routes[route];
            ...
            var fn = function($scope) {
                $scope = _self.$scope || $scope;
                $scope.$on('$locationChangeSuccess', function (event, next, last) {
                    ...
                    var callback = function(answer){
                        if(answer){
                            next && _self._applyListener($scope, next);
                            last && _self._applyListener($scope, last, true);
                        } else {
                            if(_self.errorPage){
            ...
            this.angModule.run(["$rootScope", function($rootScope){
                if(_self.$scope){
                    fn(_self.$scope);
                } else {
                    fn($rootScope);
                }
                _self._applyListener(_self.$scope || $rootScope,hashValue);
            }]);
}
    });
